...
<div show={issue_to_department_view =='issue_to_department_home'}>
 <div class="container-fluid">
  <div class="row">
    <div class="col-md-6">
      <h4>Issue to Department</h4>
    </div>
    <!-- <div class="col-md-6 text-xs-right">
      <div class="form-inline">
          <input type="search" name="searchIssueToDept" class="form-control" placeholder="search" onkeyup={filterIssueToDept} style="width:200px">
          <button class="btn btn-secondary text-right" disabled={loading} onclick={showIssueToDepartmentEntryForm}><i class="material-icons">add</i></button>
      </div>    
    </div> -->
  </div>
  <div class="row">
    <div class="col-md-3">
      <div class="form-group">
        ...
        <select name="selectReadStockTypeInput" class="form-control" onchange={readIssueToDepartment}>
          <option></option>
    ...
    <div class="col-md-1">
      <div class="form-group">
        <label for="gobtn"></label>
         <button class="btn btn-primary form-control" style="margin-top: 6px;" disabled={loading} onclick={readIssueToDepartment} id="gobtn">Go</button>
      </div>   
    </div>
    <div class="col-md-8 text-xs-right">
      <div class="form-inline">
          <input type="search" name="searchIssueToDept" class="form-control" placeholder="search" onkeyup={filterIssueToDept} style="width:200px;margin-right: 10px;">
          <button class="btn btn-secondary text-right" disabled={loading} onclick={showIssueToDepartmentEntryForm}><i class="material-icons">add</i></button>
      </div>    
    </div> 
  </div>
    ...
    <table class="table table-bordered">
        ...
        <tr each={c, i in pagedDataItems}>
          ...
          <td>
            ...
            <button disabled={loading} class="btn btn-secondary btn-sm" onclick={deleteIssue}><i class="material-icons">delete</i></button>
          </td>
        </tr>
...
<div show={issue_to_department_view =='issue_to_department_entry'}>
 <div class="container-fluid">
  <div class="row">
    <div class="col-md-9">
      <h4>{title} Issue to Department</h4>
    </div>
    <div class="col-md-3">
      <button class="btn btn-secondary text-right" disabled={loading} onclick={closeSaveIssueToDepartment}><i class="material-icons">close</i></button>
    </div>
  </div>
     ...
     <div class="row">
        <div class="col-sm-3">
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="issueDateInput" placeholder="DD/MM/YYYY">
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
            <label for="selectDepartmentInput">Department</label>
            <select name="selectDepartmentInput" class="form-control" style="min-width:250px">
              <option></option>
  	          <option each={departments} value={department_id}>{department}</option>
  	        </select>
          </div>
        </div>
        <div class="col-sm-3">  
        ...
        <div class="col-sm-3">  
          <div class="form-group">
            <label>Issue Number: </label> {issue_number}
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-sm-3">
          <div class="form-group">
            <label for="approveByInput">Issue By</label>
            <input type="text" class="form-control" id="approveByInput" disabled>
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
            <label for="receiveByInput">Receive By</label>
            <input type="text" class="form-control" id="receiveByInput" >
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
            <label for="requisitionNoInput">Requisition No</label>
            <input type="text" class="form-control" id="requisitionNoInput" >
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
   ...
   <div class="row">
    <table class="table table-bordered">
        ...
        <tr each={cat, i in selectedMaterialsArray} no-reorder>
          <td>{i+1}</td>
          <td>
          	<select id="{ 'chargeHeadInput' + cat.item_id }" class="form-control">
               <option></option>
               <option each={chargeheads} value={chargehead_id}>{chargehead}</option>
            </select>
          </td>
          ...
          <td show={title=='Add' || cat.action=='edit'}>
  ...
  <div class="col-sm-12">
    <button type="button" class="btn btn-primary pull-sm-right" onclick={saveIssueToDepartment}>Save changes</button>
    <button type="button" class="btn btn-secondary pull-sm-right" onclick={closeSaveIssueToDepartment} style="    margin-right: 10px;">Close</button>
  </div>
 </div>
</div> <!-- add_opening end -->

<div show={issue_to_department_view =='issue_to_department_eye'} class="container-fluid print-box">
 <div class="container-fluid">
  ...
  <center>
     <div style="font-size:17px;font-weight:bold">NTC INDUSTRIES LTD</div>
        ...
        Kolkata - 700058<br>
        Email: purchase@ntcind.com<br> <span if={stockAdjustmentView}> (Stock Adjustment) </span>
   </center><br>

  <table class="table table-bordered bill-info-table">
    <tr>
     <th style="width:100px">Issue No</th>
     <td>{issueDetails.stock_type_code}-{issueDetails.issue_no}</td>
     <th>Issue Date</th>
     <td>{issueDetails.issue_date}</td>
     <th>Department</th>
     <td>{issueDetails.department}</td>
    </tr>
    <tr>
     <th>
      <span if={!stockAdjustmentView}>Issue By</span>
      <span if={stockAdjustmentView}>Adjustment By</span>
     </th>
     <td>{issueDetails.approve_by}</td>
     <th>
      <span if={!stockAdjustmentView}>Receive By</span>
      <span if={stockAdjustmentView}>Verification By</span>
     </th>
     <td>{issueDetails.receive_by}</td>
     <th>Requisition No</th>
     <td>{issueDetails.requisition_no}</td>
    </tr>
  </table>
  ...
  <p show={showRemarks}><br><br><br>
    <b>Remarks: </b> {issueDetails.remarks}</p>

  <br><br>
   <table class="table indent-footer">
     <tr>
       <td style="width:50%"><center style="height:21px">{issueDetails.approve_by}</center><div><center>Issue By</center></div></td>
       <td style="width:50%"><center style="height:21px">{issueDetails.receive_by}</center><div><center>Receive By</center></div></td>
...
<div class="modal fade" id="deleteIssueModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <center><strong>Are you sure to delete issued items</strong></center>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick={confirmDeleteIssue}>Delete</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->  
...
<script>
   ...
   self.selectedMaterial = () => {
    ...
    self.closeSaveIssueToDepartment = () => {
      self.issue_to_department_view='issue_to_department_home'
    }
    ...
    self.saveIssueToDepartment = () => {
      ...
      let str=self.issueDateInput.value;
      var d = str.split("/");
    ...
    self.filterIssueToDept = () => {
      if(!self.searchIssueToDept) return
      self.filteredIssueToDept = self.issuedItems.filter(c => {
