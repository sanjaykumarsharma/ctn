<issue-to-department-chargehead-wise>
<loading-bar if={loading}></loading-bar>
<h4 class="no-print">Issue To Department (Charge Head Wise)</h4>

<div show={issue_to_department_chargehead_wise =='issue_to_department_chargehead_wise_home'} class="no-print">
    ...
    <div class="col-md-4">
      <div class="form-group">
         <table class="table table-bordered">
          <tr>
            <th class="serial-col">#</th>
            <th><input type="search" name="searchChargeHead" class="form-control" placeholder="Search Charge Head" onkeyup={filterChargeHeads}></th>
          </tr>
          <tr each={cat, i in pagedDataItems}>
            <td><input type="checkbox" class="form-control" id="{i}" checked={cat.selected} onclick={selectChargeHead.bind(this, cat)}></td>
            <td>{cat.chargehead}</td>
          </tr>
          <tfoot>
            <tr>
              <td colspan="9">
    ...
    <div class="col-md-4">
      <div class="form-group">
        <table class="table table-bordered">
          <tr>
            <th class="serial-col">#</th>
            <th>Selected ChargeHead</th>
            <th></th>
          </tr>
          <tr each={cat, i in checkedChargeHeads}>
            <td>{i+1}</td>
            <td>{cat.chargehead}</td>
            <td>
              <button class="btn btn-secondary" disabled={loading} onclick={removeChargeHead.bind(this, cat)}><i class="material-icons">remove</i></button>
            </td>
          </tr>
        </table>
      </div>
...
<div  class="container-fluid print-box" show={issue_to_department_chargehead_wise =='issue_to_department_chargehead_wise_report'}>
...
<center>
  ...
  <div>Issue To Department (Charge Head)<br> From {issueFrom} To {issueTo}</div> <br>
</center>

<div each={m, i in mainArray} class="reportDiv" no-reorder>
  <h5>Charge Head: <b>{m.chargehead}</b></h5>
...
<script>
   ...
   self.selectChargeHead = (t,e) => {
    self.checkedChargeHeads.push(t)
    
    self.chargeheads = self.chargeheads.filter(c => {
   ...
   self.removeChargeHead = (t,e) => {
     self.checkedChargeHeads = self.checkedChargeHeads.filter(c => {
      return c.chargehead_id!=t.chargehead_id
    })
