...
require_once '../api/modules/conf.php';
     ...
     $objPDO = getConnection();
      ...
      $filename = "Reject To Party (Docket Date Wise).csv";
      ...
      $condition1='';
      if($_REQUEST['stock_type_code']!='')
      $condition1='and stock_type_code in ('.$_REQUEST['stock_type_code'].')';
         ...
         $query = "select reject_to_party_id, a.item_id, item_name, item_description, docket_id,
           ...
           $transactions=$statement->fetchAll();
            ...
            $mainArray=array(); 
           foreach ($returnArray as $key => $value) { //Reject details array
              ...
              foreach ($value as $key1 => $value1) { //array of all issues of one date
                  ...
                  foreach ($transactions as $key2 => $value2) { //Reject transactions
      ...
      foreach ($mainArray as $key => $value) {
        ...
        foreach ($value['issues'] as $k => $v) {
          ...
          $obj['docket_date']='Docket Date: '.$v['rejectDetails']['docket_date'];
