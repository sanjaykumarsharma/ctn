...
require_once '../api/modules/conf.php';
     ...
     $objPDO = getConnection();
      ...
      $filename = "Issue To Department Chargehead Wise.csv";
      ...
      if($_REQUEST['chargehead_id']!='')
      $chargehead_condition='and chargehead in ('.$_REQUEST['chargehead_id'].')';
      ...
      $heading1=array('',  '',  '',  'Issue To Department Chargehead Wise', '', '', '', '');
      $heading2=array('',  '',  '',  'From ' .$_REQUEST['start_date'], 'To '.$_REQUEST['end_date'], '', '', '');
      fputcsv($fp, $heading1);
      fputcsv($fp, $heading2);
      $header=array('Serial Number', 'Issue No', 'Issue Date',  'Issue By',  'Receive By', 'Item', 'Department', 'Unit', 'Qty', 'Rate', 'Amount', 'Location');
      $blankLine=array();
      //$report_total=0;
      foreach ($mainArray as $key => $value) {
        ...
        $obj=array();
        $obj['chargehead']=$value['chargehead'];
        fputcsv($fp, $obj);
        fputcsv($fp, $blankLine);     
        fputcsv($fp, $header);       
          $count=0;
