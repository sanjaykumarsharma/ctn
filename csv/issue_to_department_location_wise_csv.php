...
require_once '../api/modules/conf.php';
     ...
     $objPDO = getConnection();
      ...
      $filename = "Issue To Department Location Wise.csv";
      ...
      $location_condition='and b.location in ('.$_REQUEST['location_id'].')';
      ...
      $heading1=array('',  '',  '',  'Issue To Department (Location Wise)', '', '', '', '');
      $heading2=array('',  '',  '',  'From ' .$_REQUEST['start_date'], 'To '.$_REQUEST['end_date'], '', '', '');
      fputcsv($fp, $heading1);
      fputcsv($fp, $heading2);
      $header=array('Serial Number', 'Issue No', 'Issue Date',  'Issue By',  'Receive By', 'Item', 'Department', 'Unit', 'Qty', 'Rate', 'Amount', 'Chargehead');
      $blankLine=array();
      //$report_total=0;
