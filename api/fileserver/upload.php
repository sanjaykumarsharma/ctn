...
if(!in_array($extension,$imgExtension)){
  ...
  $sFileSize = bytesToSize1024($_FILES['uploadedFile']['size'], 1);
     ...
     try{
         ...
         if(@move_uploaded_file($_FILES["uploadedFile"]["tmp_name"], $target . "/" . $newFileName)){
            $fn = $newFileName;
            $optimalHeight = 64;
            $source_image_path = $target . '/' . $fn;
            $thumbnail_image_path = $target . "/thumbs/" . $fn;
